...
use pocketmine\network\mcpe\protocol\types\inventory\{stackrequest\ItemStackRequestActionType,
    stackrequest\PlaceStackRequestAction,
	...
	public function handleDataPacket(DataPacketReceiveEvent $event)
	{
		...
		if ($packet instanceof ItemStackRequestPacket && $player !== null) {
			...
			if (isset(TradeDataPool::$windowIdData[$player->getName()])) {
				foreach ($packet->getRequests() as $key => $value){
					foreach ($value->getActions() as $action) {
						if ($action->getTypeId() === ItemStackRequestActionType::PLACE and $action instanceof PlaceStackRequestAction) {
							$slot = $action->getSource()->getSlotId();
							$slots[] = $slot;
						}
					}
				}
				...
				if (count($slots) === 2) {	
					$slot1 = $slots[0];
					$slot2 = $slots[1];			
					$item1 = $inv->getItem($slot1);
					$item2 = $inv->getItem($slot2);		
					$data = TradeDataPool::$interactNPCData[$player->getName()]->getShopCompoundTag()->getListTag("Recipes");
					$count = $data->getCount() - 1;					
					for ($i = 0; $i <= $count; $i++) {
						$datas = $data->get($i);
						if ($datas instanceof CompoundTag) {
							$buya = Item::nbtDeserialize($datas->getCompoundTag("buyA"));
							$buyb = Item::nbtDeserialize($datas->getCompoundTag("buyB"));
							$sell = Item::nbtDeserialize($datas->getCompoundTag("sell"));
							$trade[] = ["A" => $buya, "B" => $buyb, "C" => $sell];
						}
					}	
					foreach ($trade as $value) {
						if ($item1->equals($value["A"]) && $item2->equals($value["B"]) || $item1->equals($value["B"]) && $item2->equals($value["A"])) {
							$inv->setItem($slot1, (new ItemFactory())->get(0));
							$inv->setItem($slot2, (new ItemFactory())->get(0));
							$inv->addItem($value["C"]);
							break;
						}
					}			
		...
		if ($packet instanceof ContainerClosePacket) {
			if (isset(TradeDataPool::$windowIdData[$player->getName()])) {
				...
				unset(TradeDataPool::$windowIdData[$player->getName()]);
			}
		}
	}
